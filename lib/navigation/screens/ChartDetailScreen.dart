...
class _ChartDetailScreenState extends State<ChartDetailScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CU.getAppbar(context, widget.data[CS.gamename] ?? "Detail"),
      body: Container(
                ...
                Column(
                  children: [
                    Row(children: [
                      ...
                      if ((i + 1) < lstWin.length)
                        getWin(lstWin[i + 1][CS.result], 1)
                      else
                        getWin("", 1),
                      if ((i + 2) < lstWin.length)
                        getWin(lstWin[i + 2][CS.result], 1)
                      else
                        getWin("", 1),
                      if ((i + 3) < lstWin.length)
                        getWin(lstWin[i + 3][CS.result], 1)
                      else
                        getWin("", 1),
                      if ((i + 4) < lstWin.length)
                        getWin(lstWin[i + 4][CS.result], 1)
                      else
                        getWin("", 1),
                      if ((i + 5) < lstWin.length)
                        getWin(lstWin[i + 5][CS.result], 1)
                      else
                        getWin("", 1),
                      if ((i + 6) < lstWin.length)
                        getWin(lstWin[i + 6][CS.result], 1)
                      else
                        getWin("", 1),
                    ]),
                    Container(
...
//      CS.token: await CU.getToken(),
    ...
    if (await CU.CheckInternet()) {
      resJson = await ApiClient.Call(context,
          body: body,
          apiUrl: CS.lstgameresultlist + widget.data[CS.gameid].toString(),
