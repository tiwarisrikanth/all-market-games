...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> IsloginUser() async {
    ...
    if ((UserInfo == null ||
        CU.IsEmptyOrNull(UserInfo[k_clientid].toString()))) {
      callService();
    } else {
...
//      CS.appVersion: AppVersion,
    ...
    if (await CU.CheckInternet()) {
      resJson = await ApiClient.Call(context,
          body: body,
          apiUrl: CS.organizationDataSave,
          isShowPogressDilog: false,
    ...
    if (resJson[CS.status].toString() == StatusCode.Success) {
      if (resJson[k_lstorganizationdata] != null &&
          resJson[k_lstorganizationdata].length > 0) {
