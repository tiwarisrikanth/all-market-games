...
class GameTicketDetailScreen extends StatefulWidget {
  ...
  @override
  _GameTicketDetailScreenState createState() =>
      _GameTicketDetailScreenState(data: data);
}
...
class _GameTicketDetailScreenState extends State<GameTicketDetailScreen> {
        ...
        child: ListView(
          children: [
            ...
            Container(
              margin: EdgeInsets.all(4),
              child: Text(
                widget.data[CS.gamename].toString() +
                    " ( " +
                    widget.data[CS.openclose].toString() +
                    " )",
                textAlign: TextAlign.center,
                style: TextStyle(
                    fontSize: 14,
  ...
  getTable() {
    ...
    for (int i = 0; lstItem != null && i < lstItem.length; i++)
      WinAmt += lstItem[i][CS.winningamount] ?? 0;
    return Container(
              ...
              for (int i = 0; lstItem != null && i < lstItem.length; i++)
                Column(
                  children: [
                    Row(children: [
                      getCell(
                          CU.IsEmptyOrNull(lstItem[i][CS.number].toString())
                              ? "-"
                              : lstItem[i][CS.number].toString(),
                          1),
                      getCell(
                          CU.IsEmptyOrNull(lstItem[i][CS.number2].toString())
                              ? "-"
                              : lstItem[i][CS.number2].toString(),
                          1),
                      getCell(
                          CU.IsEmptyOrNull(data['purchaseamount'].toString())
                              ? "-"
                              : data['purchaseamount'].toString() ?? "",
                          2),
...
//      CS.token: await CU.getToken(),
    ...
    if (await CU.CheckInternet()) {
      resJson = await ApiClient.Call(context,
          body: body,
          apiUrl:
              CS.lstpurchaseitemlist + widget.data[CS.purchaseid].toString());
    } else {
