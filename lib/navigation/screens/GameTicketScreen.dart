...
class _GameTicketScreenState extends State<GameTicketScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CU.getAppbar(context, "View Ticket"),
      body: Container(
                ...
                Column(
                  children: [
                    Row(children: [
                      ...
                      getCell(
                          lstTicket[i][CS.gamename].toString() +
                              "- " +
                              lstTicket[i][CS.openclose].toString(),
                          2),
...
//      CS.token: await CU.getToken(),
    ...
    if (await CU.CheckInternet()) {
      resJson = await ApiClient.Call(context,
          body: body,
          apiUrl: CS.lstpurchasedelete + purchaseId.toString(),
          isShowPogressDilog: true);
    } else {
      CU.showNoInternetDiloag(context, callService);
      return;
