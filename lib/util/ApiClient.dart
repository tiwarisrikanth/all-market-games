...
class ApiClient {
  static Future<Map<String, dynamic>> Call(context,
      {@required apiUrl, body, isBack, isShowPogressDilog, callMethod}) async {
...
//    }
    ...
    if (response != null && response.statusCode == 200) {
      Map<String, dynamic> resjson = response.data.runtimeType == String
          ? jsonDecode(response.data)
          : response.data;
      if (resjson[CS.status] == "1" &&
          !CU.IsEmptyOrNull(resjson[k_walletamount])) {
...
//        await CU.setToken(resjson[CS.token]);
      ...
      if (resjson[CS.status].toString() == "2") {
        await showDialog(
            ...
            context: context,
            builder: (contex) {
              return CU.showAuthenticationFail(
                  context,
